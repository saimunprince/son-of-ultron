...
| Inputs | python -m syrax.bench; experiment {hypothesis, baseline, candidate, metric, repeats}; WS benchmarks, experiments; WS quality_run {only?}, quality_runs; compare_versions {base, candidate} |
...
| Code | backend/syrax/bench.py; backend/syrax/experiments.py; backend/syrax/quality.py; backend/syrax/versions.py |
| Tests | backend/syrax/test_bench.py; backend/syrax/test_experiments.py; backend/syrax/test_autonomy.py (self-improvement); backend/syrax/test_bridge.py (experiments section); backend/syrax/test_quality.py; backend/syrax/test_versions.py |
